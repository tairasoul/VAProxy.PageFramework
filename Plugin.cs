...
namespace UIWindowPageFramework
{
	...
	[BepInPlugin("va.proxy.uiwindowpage.framework", "UIWindowPages", "1.1.1")]
	public class Plugin: BaseUnityPlugin
	{
		...
		Stream GetAssets()
		{
			...
			return assembly.GetManifestResourceStream("UIWindowPageFramework.ui.textures.bundle");
		}
		...
		internal void Awake()
		{
			...
			assets = AssetBundle.LoadFromStream(GetAssets());
			LoadDotSprite();
			...
			SceneManager.activeSceneChanged += (Scene oldS, Scene New) =>
			{
				...
				if (CurrentScene != "Intro" && CurrentScene != "Menu" && GameObject.Find("MAINMENU/Canvas/Pages"))
				{
					foreach (GameObject RegisteredWindow in RegisteredWindows.Windows.Keys)
					{
						...
						Pages.pages = Pages.pages.AddItem(Instantiated.AddComponent<UIWindowPage>()).ToArray();
		...
		internal void LoadDotSprite()
		{
			...
			Spritemap = assets.LoadAsset<Texture2D>("assets/UITextures/Spritemap.png");
